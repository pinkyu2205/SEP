...
import { BigStat, Note, Panel, formatVND, shortVND } from './pricingBreakdown';
...
export const RepricingReview = ({ propertyId, sessions }: {
  ...
  const units: Unit[] = !calc ? [] : wholeHouse
    ...
    : (calc.roomResults ?? []).map((r) => {
      ...
      const newMonthly = newItems.reduce((s, i) => {
        if (i.roomId == null) return s + i.monthlyAmount / Math.max(roomCount, 1);
        return i.roomId === r.roomId ? s + i.monthlyAmount : s;
      }, 0);
      return {
        key: r.roomId,
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mb-5">
        <Panel title={`Đợt cải tạo ${session?.sessionNumber ?? ''} làm gì`} icon={Hammer}
          subtitle="Nâng cấp làm căn nhà tốt hơn nên tính vào giá. Thay đồ hỏng bằng loại tương đương chỉ giữ đúng chất lượng đã hứa với khách nên công ty tự chịu.">
          <div className="grid gap-3 sm:grid-cols-3">
            <BigStat label="Tổng chi đợt này" value={formatVND(sessionTotal)}
              sub={`Cải tạo ${shortVND(sessionRenovationCost)} · thiết bị ${shortVND(sessionEquipmentCost)}`} />
            <BigStat label="Tính vào giá" value={calc ? formatVND(intoPrice) : '—'} tone="indigo"
              sub="Chia đều cho số tháng thuê còn lại" />
            <BigStat label="Công ty tự chịu" value={calc ? formatVND(equivalent) : '—'} tone="amber"
              ...
              <tbody className="divide-y divide-slate-100">
                  ...
                  return (
                    <tr key={`l-${l.id}`}>
                      <td className="px-3 py-2">
                        <p className="font-semibold text-slate-700">Cải tạo · {l.categoryName}</p>
                ...
                {sessionEquipments.map((e) => {
                  ...
                  return (
                    <tr key={`e-${e.id}`}>
                      <td className="px-3 py-2">
                        <p className="font-semibold text-slate-700">{e.catalogName}</p>
                        {e.note && <p className="text-[11px] text-slate-400">{e.note}</p>}
                      </td>
                      <td className="px-3 py-2 text-xs text-slate-500">
                        {e.roomNumber ? `Phòng ${e.roomNumber}` : e.houseArea ? 'Khu vực chung' : 'Cả căn'}
                      </td>
                      ...
                      <td className="px-3 py-2 text-xs">
                        {!calc ? '—' : !it
                          ...
                          : it.kind === 'EQUIPMENT_UPGRADE'
                            ...
                            : <span className="font-semibold text-indigo-700">Thêm mới · tính vào giá</span>}
                      </td>
                    </tr>
                  );
                })}
      ...
      <div className="mb-5">
        <Panel title={wholeHouse ? 'Giá niêm yết mới của căn' : `Giá niêm yết mới từng phòng (${units.length} phòng)`} icon={Banknote}
          subtitle={`Gợi ý = giá đang niêm yết + phần tăng do đợt này (khấu hao mới ÷ ${Math.round((1 - vRate) * 100)}% lấp đầy). Sửa được từng ô.`}>
          {!calc ? (
                ...
                <table className="w-full min-w-[900px] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {units.map((u) => {
                      ...
                      return (
                        <tr key={u.key} className={below ? 'bg-rose-50/40' : undefined}>
                          <td className="px-3 py-2.5 font-bold text-slate-800">{u.label}</td>
              ...
              <div className="space-y-2 text-sm text-slate-600">
                ...
                {absorbedByTenants > 0 && (
                  <p className="text-xs text-slate-500">
                    Phần tăng do đợt này mà khách đang ở chưa trả, cho tới khi hợp đồng của họ hết hạn:{' '}
                    <b className="text-amber-700">khoảng {formatVND(absorbedByTenants)}</b>{' '}
                    ({rented.map((u, i) => (
                      <span key={u.key}>{i > 0 && ' · '}{u.label} {shortVND(u.increase)} × {monthsUntil(u.contract?.endDate)} tháng</span>
                    ))}). Công ty tự chịu khoản này.
                  </p>
                )}
              </div>
            )}
      ...
      <div className="sticky bottom-0 z-30 -mx-4 mt-5 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-4px_16px_rgba(15,23,42,0.06)] backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Giá niêm yết mới / tháng</p>
