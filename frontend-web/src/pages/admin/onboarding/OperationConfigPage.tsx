...
import { RenovationCumulativeView, equipLoc } from './RenovationCumulativeView';
...
const RenovationHistoryPanel = ({ property }: { property: PropertyResponse }) => {
  ...
  const [view, setView] = useState<'total' | 'sessions'>('total');
  ...
  // `totalCost` của BE chỉ cộng HẠNG MỤC CẢI TẠO, không có thiết bị mua — ghi rõ ở nhãn thẻ.
  const renovationTotal = sessions.reduce((s, sess) => s + sess.totalCost, 0);

  return (
    <div className="space-y-5">
      {/* Hai cách xem: tổng gộp mọi đợt (mặc định) hoặc từng đợt v1, v2… */}
      <div className="flex flex-wrap items-center gap-3">
        <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1">
          {([
            ['total', 'Tổng đến hiện tại'],
            ['sessions', `Từng đợt${sessions.length ? ` (${[...sessions].sort((a, b) => a.sessionNumber - b.sessionNumber).map(s => s.versionLabel ?? `v${s.sessionNumber}`).join(', ')})` : ''}`],
          ] as const).map(([key, label]) => (
            <button key={key} type="button" onClick={() => setView(key)}
              className={`rounded-lg px-3.5 py-1.5 text-sm font-bold transition ${
                view === key ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800'}`}>
              {label}
            </button>
          ))}
        </div>
        <p className="text-xs text-slate-400">
          {view === 'total'
            ? 'Gộp mọi đợt: tổng vốn, lũy kế theo thời gian, hạng mục theo loại, thiết bị theo vị trí.'
            : 'Mỗi đợt một khối — đợt đang hiệu lực mở sẵn.'}
        </p>
      </div>
      ...
      {sessions.length === 0 ? (
        ...
        <RenovationCumulativeView sessions={sessions} />
      ) : (
        <>
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
              <p className="text-xs font-bold text-amber-700/70 uppercase tracking-wide mb-1.5">Chi phí cải tạo tất cả đợt</p>
              <p className="text-2xl font-black text-amber-600">{formatVND(renovationTotal)}</p>
              <p className="mt-0.5 text-xs text-amber-700/70">Chưa gồm thiết bị mua — xem tab “Tổng đến hiện tại”</p>
            </div>
...
/**
 * MÃ KHÁCH HÀNG ĐIỆN / SỐ DANH BỘ NƯỚC — CHỈ XEM.
 *
 * Chốt 17/09/2026: hai mã này là dữ liệu nhập từ file Khởi tạo nhà, KHÔNG sửa tay trên giao
 * diện. Bản 11/09 từng có ô nhập + nút "Lưu mã" (PATCH .../utility-customer-codes) để chữa khi
 * file gõ nhầm — đã gỡ: nhìn như dữ liệu import sửa tuỳ ý được, lại dễ lỡ tay gõ vào, mà mã sai
 * là máy chủ chặn phát hành hoá đơn của cả căn.
 *
 * Đường đổi mã duy nhất: sửa cột "Mã khách hàng điện" / "Mã khách hàng nước" trong file khởi
 * tạo nhà rồi NHẬP LẠI. Máy chủ nhận ra nhà đã tồn tại (trùng mã HĐ) và chỉ ghi đè hai mã —
 * kết quả `CODES_UPDATED`, không tạo nhà mới, không đụng trường nào khác.
 *
 * Hiện IN HOA cho dễ dò với tờ hoá đơn giấy; máy chủ lưu chữ thường sau khi chuẩn hoá (bỏ dấu
 * cách/gạch), đối chiếu thì hai bên đều chuẩn hoá nên hoa hay thường không đổi kết quả.
 */
const UtilityCodeCard = ({ detail }: { detail: PropertyResponse }) => {
  const elec = detail.electricityCustomerCode?.trim();
  const water = detail.waterCustomerCode?.trim();

  const CodeRow = ({ label, value }: { label: string; value?: string }) => (
    <div>
      <p className="mb-0.5 text-xs font-bold text-slate-600">{label}</p>
      {value ? (
        <p title="Lấy từ file Khởi tạo nhà. Đổi mã: sửa cột mã trong file rồi nhập lại."
          className="break-all rounded-lg bg-slate-50 px-3 py-1.5 font-mono text-sm font-bold uppercase text-slate-900 ring-1 ring-slate-200">
          {value}
        </p>
      ) : (
        <p className="rounded-lg bg-amber-50 px-3 py-1.5 text-sm font-bold text-amber-700 ring-1 ring-amber-200">Chưa khai</p>
      )}
    </div>
  );
  ...
  return (
    <SideCard title="Mã khách hàng" icon={Hash}>
      <div className="space-y-3">
        <CodeRow label="Mã khách hàng điện" value={elec} />
        <CodeRow label="Số danh bộ nước" value={water} />

        {/* Thiếu mã KHÔNG chặn gì cả — nó chỉ lặng lẽ tắt việc đối chiếu, tức mất cái chốt bắt
            hoá đơn gắn nhầm nhà mà không ai hay. Nên phải nói ra. */}
        {(!elec || !water) && (
